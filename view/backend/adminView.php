...
<article class="news col-3 postDetails">
    ...
    <form onsubmit="return confirm('Voulez vous supprimer le chapitre ?')" 
          action="index.php?action=removeChapter&amp;id=<?= $post['id'] ?>" method="POST" >
    ...
    <h2 class="adminView">Commentaires</h2>
        ...
        <?php foreach ($comments as $comment) { ?>
            ...
            <?php if($comment['reported'] != 0){ ?>
                <strong>Commentaire signalé <?= $comment['reported'] ?> fois </strong><br /><!--show on adminView the number of reports existing & depending of the comment-->
            <?php }    
        } ?>
</div>
